...
BEGIN
    ...
    USING (
        SELECT * FROM (
            SELECT
                SHOPPERTRAK_SITE_ID, SITE_NAME, BOROUGH, ADDRESS, CITY, STATE, POSTCODE, LAT, LON, SITE_CATEGORY,
    ...
    WHEN MATCHED AND t._RAW_HASH <> s._RAW_HASH THEN UPDATE SET
        ...
        LAT = s.LAT, LON = s.LON, SITE_CATEGORY = s.SITE_CATEGORY,
        _EFFECTIVE_TS = s._EFFECTIVE_TS, _LOAD_TS = s._LOAD_TS, _RAW_HASH = s._RAW_HASH
    WHEN NOT MATCHED THEN INSERT (
        SHOPPERTRAK_SITE_ID, SITE_NAME, BOROUGH, ADDRESS, CITY, STATE, POSTCODE, LAT, LON, SITE_CATEGORY,
        _EFFECTIVE_TS, _LOAD_TS, _RAW_HASH
    ) VALUES (
        s.SHOPPERTRAK_SITE_ID, s.SITE_NAME, s.BOROUGH, s.ADDRESS, s.CITY, s.STATE, s.POSTCODE, s.LAT, s.LON, s.SITE_CATEGORY,
