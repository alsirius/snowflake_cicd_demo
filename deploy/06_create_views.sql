...
-- NEW VIEW: Site Category Analytics
CREATE OR REPLACE VIEW PRS.V_SITE_CATEGORY_SUMMARY AS
SELECT
    SITE_CATEGORY,
    BOROUGH,
    COUNT(DISTINCT SHOPPERTRAK_SITE_ID) AS SITE_COUNT,
    ...
    SUM(EXITS)::int AS TOTAL_EXITS,
    SUM(NET_VISITS)::int AS TOTAL_NET_VISITS,
    AVG(DAILY_AVG_VISITS) AS AVG_DAILY_VISITS
FROM PRS.V_DAILY_SITE_VISITS
JOIN INT.DIM_SITE USING (SHOPPERTRAK_SITE_ID)
GROUP BY 1, 2
ORDER BY SITE_CATEGORY, TOTAL_NET_VISITS DESC;
